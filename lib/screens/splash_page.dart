...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _loadData() async {
    ...
    // 2. 모든 만다라트 상세 정보 가져오기
    await Future.wait(mandalarts.map((mandalart) async {
      final mandalartId = mandalart['id'] ?? '0';
      final info = await UserMandaInfoService.userMandaInfo(context, mandalartId: int.parse(mandalartId));
      if (info != null) {
        ...
        if (status == "IN_PROGRESS") inProgressIDs.add({"id": id, "name": name});
        if (status == "SUCCESS") successIDs.add({"id": id, "name": name});
      }
    }));

    // 3. MainGoal, EmptyMainGoal 처리
    ...
    for (var goal in inProgressIDs) {
      ...
      final data = await SecondGoalListService.secondGoalList(context, mandalartId);
      final secondGoalData = data?[0]['secondGoals'] as List<Map<String, dynamic>>?;
      if (secondGoalData != null && secondGoalData.isNotEmpty) {
        mainGoals.add(goal);
      } else {
        emptyMainGoals.add({'mandalartId': mandalartId, 'name': goal['name']});
      }
      if (data != null) secondGoals.addAll(data);
    }
